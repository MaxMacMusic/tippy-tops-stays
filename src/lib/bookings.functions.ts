...
function publicClient() {
  // Server env first, then the build-time public vars so the site keeps working
  // on hosts where only the VITE_* variables are configured.
  const url =
  ...
  if (!url || !key) {
    throw new Error(
      "Supabase is not configured on the server (SUPABASE_URL / SUPABASE_PUBLISHABLE_KEY).",
    );
  }
