...
RETURNS TABLE(id uuid, nights int, total_aud int)
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.create_booking(text, text, text, date, date, int, int, int, text, text) FROM public;
GRANT EXECUTE ON FUNCTION public.create_booking(text, text, text, date, date, int, int, int, text, text) TO anon, authenticated;
